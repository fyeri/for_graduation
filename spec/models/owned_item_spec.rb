  ...
  describe '検索機能' do
    ...
    context 'scopeメソッドでラベルを検索した場合' do
      it 'ラベルに完全一致した商品が絞り込まれる' do
        expect(OwnedItem.with_label_name('Label1')).to include(owned_item1)
        expect(OwnedItem.with_label_name('Label1')).not_to include(owned_item2)
        expect(OwnedItem.with_label_name('Label1')).not_to include(owned_item3)
        expect(OwnedItem.with_label_name('Label1').count).to eq 1
      end
    end
    context 'scopeメソッドで商品名、キャラクター名の曖昧検索とラベル検索をした場合' do
      it '検索ワードを含んだ商品名、キャラクター名、かつラベルに完全一致する商品が絞り込まれる' do
        expect(OwnedItem.with_item_name('first').with_item_character('A').with_label_name('Label1')).to include(owned_item1)
        expect(OwnedItem.with_item_name('first').with_item_character('A').with_label_name('Label1')).not_to include(owned_item2)
        expect(OwnedItem.with_item_name('first').with_item_character('A').with_label_name('Label1')).not_to include(owned_item3)
        expect(OwnedItem.with_item_name('first').with_item_character('A').with_label_name('Label1').count).to eq 1
      end
    end
  end
end
